...
function renderContent(raw: string) {
  ...
  if (raw.trim().startsWith("<")) {
    return (
      <div
        className="rte-policy-output text-muted-foreground leading-relaxed"
...
export default function PrivacyPolicy() {
  ...
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => r.json())
      .then((d) => { if (d?.privacyPolicy) setPolicy(d.privacyPolicy); })
      .catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-[#0033A0] text-white py-16 px-6 md:px-12">
        <div className="max-w-3xl mx-auto">
          ...
          {policy.lastUpdated && (
            <p className="text-white/50 text-sm">Last updated: {policy.lastUpdated}</p>
          )}
        </div>
      </div>
